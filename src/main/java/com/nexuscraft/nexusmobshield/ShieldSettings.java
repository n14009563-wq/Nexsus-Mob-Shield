...
public final class ShieldSettings {
    ...
    private boolean silenceCommandBlockOutput = true;
    ...
    public void load() {
        ...
        if (settings != null) {
            silenceCommandBlockOutput = settings.getBoolean("silence-command-block-output", true);
        }
    }
    ...
    /** Whether this plugin should force the vanilla commandBlockOutput gamerule off itself
     * (on enable/reload and whenever a world loads), instead of relying on an admin running
     * /gamerule by hand -- see /nexusmobshield quiet. */
    public boolean isSilenceCommandBlockOutput() {
        return silenceCommandBlockOutput;
    }

    public void setSilenceCommandBlockOutput(boolean value) {
        this.silenceCommandBlockOutput = value;
        plugin.getConfig().set("settings.silence-command-block-output", value);
        plugin.saveConfig();
    }
}
