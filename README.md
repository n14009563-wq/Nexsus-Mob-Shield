...
**v0.2.0 fixed a real gap from v0.1.0: mobs were still dying even with this plugin installed.**
...
**v0.2.1 fixed the next problem once protection was confirmed working: the console was still
unusable.** Even with mobs no longer actually dying, the rogue command block's own success-message
broadcast (`"[@: Killed N entities]"`, `"[@: Successfully cloned 1 block(s)]"`) kept flooding the
console every tick -- that's a separate vanilla behavior from whether the kill succeeds. See
"Silencing the console spam" below.
...
## Silencing the console spam (v0.2.1)

Confirming protection was working (via `/nexusmobshield status` showing a large, growing "blocked"
count and zero forced recoveries) didn't fix the console itself -- it was still flooded every tick
with the command block's own `"[@: Killed N entities]"` / `"[@: Successfully cloned 1 block(s)]"`
feedback. That's just Minecraft reporting that the command *ran*, completely separate from whether
the resulting damage got cancelled. Vanilla has a dedicated switch for this broadcast --
`/gamerule commandBlockOutput false` -- but running that by hand from some server consoles/panels
can fail (this happened directly: it came back "unknown command" on this server's console).

So NexusMobShield now sets that same gamerule itself, directly through the Bukkit API, rather than
depending on an admin being able to run it manually: on every currently-loaded world at
startup/reload, and again automatically for any world that loads afterward. Controlled by
`settings.silence-command-block-output` (default `true`), and toggleable live with
`/nexusmobshield quiet <on|off>` -- current state also shows in `/nexusmobshield status`.

This changes nothing about protection or recovery -- it purely stops the noise.
...
- `/nexusmobshield quiet <on|off>` -- toggle silencing the command block console-spam gamerule
  live, on every loaded world. Persists immediately.
...
- `recovery.*` -- the "forced back into the game" safety net described above: `enabled` and
  `max-per-minute`.
- `settings.silence-command-block-output` -- see "Silencing the console spam" above. Defaults to
  `true`.
...
Produces `target/nexusmobshield-0.2.1.jar`. Requires Java 21 and network access to
