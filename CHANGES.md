...
## v0.2.1 -- fix: console still flooded even after protection was confirmed working

Built from a direct follow-up after v0.2.0 was confirmed working: `/nexusmobshield status` showed
489,651+ successful blocks and zero forced recoveries, proving mobs were no longer actually dying
-- but the console itself was still unusable, flooded every tick with the rogue command block's
own `"[@: Killed N entities]"` / `"[@: Successfully cloned 1 block(s)]"` feedback lines. That
feedback is a vanilla broadcast completely separate from whether the kill actually succeeds --
Bukkit prints it because the command *ran*, whether or not the resulting damage got cancelled.

**The fix:** vanilla Minecraft has a dedicated gamerule for this exact broadcast --
`commandBlockOutput` -- but running `/gamerule commandBlockOutput false` by hand from this
server's console panel came back "unknown command" (most likely a console/panel quirk on this
host, unclear exactly why). Rather than depend on that command working from an admin's console,
NexusMobShield now sets the same gamerule itself, directly through the Bukkit API, on every
currently-loaded world at startup and reload, and again automatically for any world that loads
afterward. New `settings.silence-command-block-output` config option (default `true`), and a new
`/nexusmobshield quiet <on|off>` command to toggle it live and see its current state instantly
via `/nexusmobshield status`.

This only silences the broadcast -- it changes nothing about protection, blocking, or recovery,
all of which were already confirmed working before this release.

**Verification:** added `GameRule`, `World.setGameRule`, `Server.getWorlds`, and a
`WorldLoadEvent` stub to cover the new surface, then recompiled the whole plugin with
